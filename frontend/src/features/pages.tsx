...
function ResumableTeacherAudioWorkflow({
  ...
  return <section className="content-section audio-workflow" aria-labelledby="audio-workflow-title">
    ...
    {revision ? <div className="transcript-review"><p className="provenance-label">{revision.provenance_label}</p>{revision.provenance_summary?.local_only ? <p>Local speech recognition · Model: {revision.provenance_summary.model_identifier} · Processed on this device · No classroom audio was sent to a cloud transcription service.{revision.provenance_summary.language_detected ? ` Detected language: ${revision.provenance_summary.language_detected}.` : ""}{revision.provenance_summary.inference_seconds !== null ? ` Processing time: ${revision.provenance_summary.inference_seconds.toFixed(1)} seconds.` : ""}</p> : null}<p>Revision {revision.revision_number}{revision.copied_from_transcript_revision_id ? " · New revision based on the previous transcript" : ""} · {revision.teacher_review_status === "APPROVED" ? "Teacher-reviewed and approved" : "Needs teacher review"}</p><ol className="transcript-segments">{revision.segments.map((segment) => <li key={segment.id}><time>{(segment.start_ms / 1000).toFixed(1)}–{(segment.end_ms / 1000).toFixed(1)}s</time><span lang="ml">{segment.text}</span></li>)}</ol>{revision.suggestions.map((suggestion) => <article className="term-suggestion" key={suggestion.id}><p><strong>{suggestion.detected_text}</strong> → {suggestion.canonical_term} {suggestion.malayalam_support_label ? <span lang="ml">/ {suggestion.malayalam_support_label}</span> : null}</p><div className="review-actions"><Button aria-pressed={suggestion.latest_decision === "CONFIRMED"} disabled={busy || !capabilities?.can_edit_transcript} onClick={() => decide(suggestion.id, "CONFIRMED")} type="button">Confirm</Button><Button aria-pressed={suggestion.latest_decision === "REJECTED"} disabled={busy || !capabilities?.can_edit_transcript} onClick={() => decide(suggestion.id, "REJECTED")} type="button">Reject</Button><Button aria-pressed={suggestion.latest_decision === "UNSURE"} disabled={busy || !capabilities?.can_edit_transcript} onClick={() => decide(suggestion.id, "UNSURE")} type="button">Unsure</Button></div></article>)}</div> : null}
...
function matchingRecoverySupport(lesson: StudentLesson, conceptId: string) {
  const matches = lesson.recovery_support.filter((pack) => pack.concept_id === conceptId);
  const pack = matches.length === 1 ? matches[0] : null;
  if (!pack) return null;
  const content = [
    pack.cue.english,
    pack.cue.malayalam,
    pack.example.english,
    pack.example.malayalam,
    pack.alternate_explanation.english,
    pack.alternate_explanation.malayalam,
  ];
  return content.every((value) => value.trim().length > 0) ? pack : null;
}
...
function FocusRecoveryPanel({
  ...
  step: ReturnType<typeof buildFocusJourneySteps>[number];
  stepIndex: number;
  ...
  const pack = matchingRecoverySupport(lesson, step.id);
  const terms = lesson.glossary_terms
    .filter((term) => term.concept_ids.includes(step.id))
...
export function FocusJourneyPage() {
  ...
  const activeRecovery = steps[progress.currentStepIndex]?.id ? progress.recoveryByConcept[steps[progress.currentStepIndex].id] : undefined;
  ...
  useEffect(() => {
    if (progress.screen === "concept" && activeRecovery?.recoveryOpened) recoveryHeadingRef.current?.focus();
  ...
  const recovery = progress.recoveryByConcept[step.id];
  const recoveryStage = recovery?.recoveryOpened ? recovery.currentRecoveryStage : null;
  const updateRecovery = (next: FocusRecoveryProgress) => {
